...
class CChromosome:
    ...
    def __init__(self, mutation_range, mutation_rate, loci = []):
    ...
    def __add__(self, other_chromosome):
        ...
        """
        novi_loci = []
        for i, (locusA, locusA_evolves) in enumerate(self.loci):
            (locusB, locusB_evolves) = other_chromosome.loci[i]
            if random.random()<0.5:
                novi_loci.append((locusA*1, locusA_evolves)) #*1 ensures that a copy of the value is made and not just a reference
            else:
                novi_loci.append((locusB*1, locusB_evolves))
